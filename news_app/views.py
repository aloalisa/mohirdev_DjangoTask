...
def news_list(request):
    news_list = News.objects.filter(status=News.Status.Published) [:5]
    context = {
        'news_list': news_list
    }
...
def news_detail(request, news):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            # yangi komment obyekti yaratamiz lekin DB ga saqlamaymiz
...
def homePageView(request):
    ...
    # local_one=News.objects.latest('publish_time')
    # local_news = News.objects.all().filter(category__name="Mahalliy")
    mahalliy_xabarlar = News.objects.all().filter(category__name='Mahalliy')
    xorij_xabarlari=News.objects.all().filter(category__name='Xorij')
    sport_xabarlari=News.objects.all().filter(category__name='Sport')
    texnologiya_xabarlari=News.objects.all().filter(category__name='Texnologiya')
...
#         context['local_one']=News.published.filter(category__name='Mahalliy')
#         context['local_news']=News.published.filter(category__name='Mahalliy')
#         return context
...
#formalarda 2 xil request bo'ladi post request va get request, post joylash, get olish qabul qilish un ish-di.
